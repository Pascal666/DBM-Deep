...
This is the size in bytes of a given data sector. The default for this is 64
bytes. The minimum value is 32 and the maximum is 256 bytes.

Keys (or array indexes) and values are each stored in their own sectors, so
each database entry consumes at least 2 * data_sector_size bytes.

Data sectors will chain, so a value of any size can be stored. However, chaining
is expensive in terms of time. Setting this value to something close to the
expected common length of your scalars will improve your performance. If it is
too small, your file will have a lot of chaining. If it is too large, your file
will have a lot of dead space in it.

B<Note:> There are between 6 and 12 bytes taken up in each data sector for
bookkeeping. (It's 4 + the number of bytes in your L</pack_size>.) This is
included within the data_sector_size, thus the usable space is 6-12 bytes
...
With a data_sector_size of 32, this will hold a maximum of 236 entries.

With a data_sector_size of 64, this will hold a maximum of 178 entries.

With a data_sector_size of 128, this will hold a maximum of 122 entries.

With a data_sector_size of 256, this will hold a maximum of 79 entries.
